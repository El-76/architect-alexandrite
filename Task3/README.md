...
Вместо изначальных service-a и service-b сделаны orders-service (сервис заказов) и models-service (сервис 3D-моделей), в каждом по одной GET-ручке, которая получает соответствующую сущность по ID, orders-service идёт за данными модели в models-service в коде ручки. Ручка в models-service с вероятностью 0,5 может "тормозить" - выполняться 1 секунду.
